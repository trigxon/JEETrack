...
function safeCompare(a, b) {
  ...
  if (aBuf.length !== bBuf.length) {
    _nodeCrypto.default.timingSafeEqual(aBuf, Buffer.alloc(aBuf.length));
    return false;
  }
  return _nodeCrypto.default.timingSafeEqual(aBuf, bBuf);
}
function signToken(payload) {
  const body = Buffer.from(JSON.stringify(payload)).toString('base64url');
  const sig = _nodeCrypto.default.createHmac('sha256', ADMIN_TOKEN_SECRET).update(body).digest('base64url');
  return `${body}.${sig}`;
}
function verifyToken(token) {
  ...
  const expectedSig = _nodeCrypto.default.createHmac('sha256', ADMIN_TOKEN_SECRET).update(body).digest('base64url');
  const sigBuf = Buffer.from(sig);
  const expBuf = Buffer.from(expectedSig);
  if (sigBuf.length !== expBuf.length || !_nodeCrypto.default.timingSafeEqual(sigBuf, expBuf)) return null;
  let payload;
  try {
...
const COACHING_LABELS = {
  pw_online: 'PW Online',
  allen_online: 'Allen Online',
  unacademy: 'Unacademy',
  vedantu: 'Vedantu',
  aakash_online: 'Aakash Digital',
  motion_online: 'Motion Online',
  other_online: 'Other (Online)',
  pw_vidyapeeth: 'PW Vidyapeeth',
  allen: 'Allen',
  aakash: 'Aakash',
  fiitjee: 'FIITJEE',
  resonance: 'Resonance',
...
async function buildRoster({
  ...
  const [authUsers, prefs] = await Promise.all([sbAuthListAllUsers().catch(() => []), sbQuery('user_preferences?select=user_id,username,class_year,coaching,study_mode,referral_source,target_year,email_reports,last_active_at,onboarding_done,created_at').catch(() => [])]);
  const prefMap = {};
  prefs.forEach(p => {
    prefMap[p.user_id] = p;
  });
  ...
  try {
    ...
    if (action === 'stats') {
      ...
      const data = await cached(`stats_${days}`, 60000, async () => {
        const activePrefs = await sbQuery(`user_preferences?select=user_id,last_active_at&last_active_at=gte.${cutoff}T00:00:00Z`).catch(() => []);
        const [totalUsers, totalTests, totalHours, totalBacklogs, totalTodos, totalFeedbacks, aiInsightsUsers] = await Promise.all([sbCount('user_preferences').catch(() => 0), sbCount('tests').catch(() => 0), sbCount('hours').catch(() => 0), sbCount('backlogs').catch(() => 0), sbCount('todos').catch(() => 0), sbCount('feedback').catch(() => 0), sbQuery(`user_preferences?select=user_id&ai_insights_count=gt.0`).catch(() => [])]);
    ...
    if (action === 'features') {
      ...
      const [testUsers, hoursUsers, backlogUsers, todoUsers, syllabusUsers, feedbackUsers, aiUsers] = await Promise.all([sbQuery(`tests?select=user_id&created_at=gte.${cutoff}T00:00:00Z`).catch(() => []), sbQuery(`hours?select=user_id&created_at=gte.${cutoff}T00:00:00Z`).catch(() => []), sbQuery(`backlogs?select=user_id&created_at=gte.${cutoff}T00:00:00Z`).catch(() => []), sbQuery(`todos?select=user_id&created_at=gte.${cutoff}T00:00:00Z`).catch(() => []), sbQuery(`syllabus?select=user_id&updated_at=gte.${cutoff}T00:00:00Z`).catch(() => []), sbQuery(`feedback?select=user_id&created_at=gte.${cutoff}T00:00:00Z`).catch(() => []), sbQuery(`user_preferences?select=user_id&ai_insights_count=gt.0&last_active_at=gte.${cutoff}T00:00:00Z`).catch(() => [])]);
      const uniq = arr => new Set(arr.map(r => r.user_id)).size;
      const features = [{
        feature: 'Mock Tests',
        users: uniq(testUsers),
        total: totalAll
      }, {
        feature: 'Study Hours',
        users: uniq(hoursUsers),
        total: totalAll
      }, {
        feature: 'Backlog',
        users: uniq(backlogUsers),
        total: totalAll
      }, {
        feature: 'To-Do',
        users: uniq(todoUsers),
        total: totalAll
      }, {
        feature: 'Syllabus',
        users: uniq(syllabusUsers),
        total: totalAll
      }, {
        feature: 'AI Insights',
        users: uniq(aiUsers),
      ...
      const dauFeatures = [{
        feature: 'Mock Tests',
        dauPct: Math.round(uniq(testUsers) / totalAll * 100),
        dau: uniq(testUsers)
      }, {
        feature: 'Study Hours',
        dauPct: Math.round(uniq(hoursUsers) / totalAll * 100),
        dau: uniq(hoursUsers)
      }, {
        feature: 'AI Insights',
        dauPct: Math.round(uniq(aiUsers) / totalAll * 100),
        dau: uniq(aiUsers)
      }, {
        feature: 'Syllabus',
        dauPct: Math.round(uniq(syllabusUsers) / totalAll * 100),
        dau: uniq(syllabusUsers)
      }];
      return res.status(200).json({
        features,
        dauFeatures,
    ...
    if (action === 'pages') {
      const cutoff = dateFrom(days);
      const [tests, hours, backlogs, todos, syllabus, feedback, ai] = await Promise.all([sbQuery(`tests?select=user_id&created_at=gte.${cutoff}T00:00:00Z`).catch(() => []), sbQuery(`hours?select=user_id&created_at=gte.${cutoff}T00:00:00Z`).catch(() => []), sbQuery(`backlogs?select=user_id&created_at=gte.${cutoff}T00:00:00Z`).catch(() => []), sbQuery(`todos?select=user_id&created_at=gte.${cutoff}T00:00:00Z`).catch(() => []), sbQuery(`syllabus?select=user_id&updated_at=gte.${cutoff}T00:00:00Z`).catch(() => []), sbQuery(`feedback?select=user_id&created_at=gte.${cutoff}T00:00:00Z`).catch(() => []), sbQuery(`user_preferences?select=user_id&ai_insights_count=gt.0&last_active_at=gte.${cutoff}T00:00:00Z`).catch(() => [])]);
      const uniq = arr => new Set(arr.map(r => r.user_id)).size;
      return res.status(200).json([{
        page: 'Mock Tests',
        count: uniq(tests)
    ...
    if (action === 'subjects') {
      const [phys, chem, math] = await Promise.all([sbCount('hours', 'subject=eq.physics').catch(() => 0), sbCount('hours', 'subject=eq.chemistry').catch(() => 0), sbCount('hours', 'subject=eq.maths').catch(() => 0)]);
      return res.status(200).json([{
        subject: 'physics',
        count: phys
      }, {
    ...
    if (action === 'exams') {
      const [mains, advanced] = await Promise.all([sbCount('tests', 'exam=eq.mains').catch(() => 0), sbCount('tests', 'exam=eq.advanced').catch(() => 0)]);
      return res.status(200).json([{
        exam: 'mains',
        count: mains
      }, {
    ...
    if (action === 'funnel') {
      const [totalUsers, usersWithTests, usersWithHours, usersWithBacklogs, usersWithSyllabus, usersWithAI] = await Promise.all([sbCount('user_preferences').catch(() => 0), sbQuery('tests?select=user_id').catch(() => []), sbQuery('hours?select=user_id').catch(() => []), sbQuery('backlogs?select=user_id').catch(() => []), sbQuery('syllabus?select=user_id').catch(() => []), sbQuery('user_preferences?select=user_id&ai_insights_count=gt.0').catch(() => [])]);
      ...
      return res.status(200).json([{
        ...
        event: 'mock_test_logged',
        label: 'Logged Mock Test',
        count: distinctTestUsers
      }, {
        event: 'study_hours_logged',
        label: 'Logged Study Hours',
        count: distinctHoursUsers
      }, {
        event: 'backlog_used',
        label: 'Used Backlog',
        count: distinctBacklogUsers
      }, {
        event: 'syllabus_used',
        label: 'Used Syllabus Tracker',
        count: distinctSyllabusUsers
      }, {
        event: 'ai_insights_generated',
        label: 'Used AI Insights',
        count: distinctAIUsers
      }]);
    }
    if (action === 'users') {
    ...
    if (action === 'demographics') {
      ...
      raw.forEach(r => {
        ...
        if (!byDim[dim]) return;
        const label = dim === 'class' ? classLabel(r.label) : dim === 'coaching' ? coachingLabel(r.label) : dim === 'source' ? sourceLabel(r.label) : r.label || 'Not Set';
        byDim[dim][label] = (byDim[dim][label] || 0) + Number(r.cnt);
        if (dim === 'class') total += Number(r.cnt);
      });
      const toArr = obj => Object.entries(obj).map(([label, count]) => ({
        label,
        count,
    ...
    if (action === 'user_detail') {
      ...
      if (!distinct_id) return res.status(400).json({
        error: 'distinct_id required'
      });
      const [testsData, hoursData, syllabusData, backlogs, todos, feedbacks, streaks, prefs, authUser] = await Promise.all([sbQuery(`tests?select=*&user_id=eq.${distinct_id}&order=created_at.desc`).catch(() => []), sbQuery(`hours?select=*&user_id=eq.${distinct_id}&order=date.desc`).catch(() => []), sbQuery(`syllabus?select=*&user_id=eq.${distinct_id}`).catch(() => []), sbCount('backlogs', `user_id=eq.${distinct_id}`).catch(() => 0), sbCount('todos', `user_id=eq.${distinct_id}`).catch(() => 0), sbQuery(`feedback?select=*&user_id=eq.${distinct_id}&order=created_at.desc`).catch(() => []), sbQuery(`streaks?select=*&user_id=eq.${distinct_id}`).catch(() => []), sbQuery(`user_preferences?select=*&user_id=eq.${distinct_id}`).catch(() => []), sbAuthGetUser(distinct_id).catch(() => null)]);
      ...
      const avgMaxPct = totalTests ? Math.round(testsData.reduce((s, t) => s + (t.max ? (t.total || 0) / t.max * 100 : 0), 0) / totalTests) : 0;
      ...
      const activityDates = new Set([...hoursData.filter(h => h.date).map(h => h.date), ...testsData.filter(t => t.date).map(t => t.date)]);
    ...
    if (action === 'leaderboard') {
      ...
      if (!roster.length) return res.status(200).json({
        leaderboard: [],
        topper: null
      });
      const [allTests, allHours] = await Promise.all([sbQuery('tests?select=user_id,total,max,physics,chemistry,maths,exam,date').catch(() => []), sbQuery('hours?select=user_id,total,subject,date').catch(() => [])]);
      const userTestMap = {};
      allTests.forEach(t => {
      ...
      const entries = roster.map(u => {
        ...
        const avgScorePct = tests.length ? Math.round(tests.reduce((s, t) => s + (t.max ? (t.total || 0) / t.max * 100 : 0), 0) / tests.length) : 0;
    ...
    if (action === 'consistency') {
      ...
      const [allTests, allHours] = await Promise.all([sbQuery(`tests?select=user_id,date&date=gte.${dateFrom(180)}`).catch(() => []), sbQuery(`hours?select=user_id,date&date=gte.${dateFrom(180)}`).catch(() => [])]);
      const userDates = {};
    ...
    if (action === 'save_site_config') {
      ...
      try {
        ...
        const inserted = await sbQuery('app_config', 'POST', {
          id: 1,
          ...payload
        });
        return res.status(200).json({
    ...
    if (action === 'db_stats') {
      const data = await cached('db_stats', 60000, async () => {
        const [tests, hours, backlogs, todos, syllabus, feedbackCount, prefs] = await Promise.all([sbCount('tests').catch(() => 0), sbCount('hours').catch(() => 0), sbCount('backlogs').catch(() => 0), sbCount('todos').catch(() => 0), sbCount('syllabus').catch(() => 0), sbCount('feedback').catch(() => 0), sbQuery('user_preferences?select=user_id,email_reports,last_active_at').catch(() => [])]);
    ...
    if (action === 'feedback_list') {
      ...
      try {
        all = await sbQuery(`feedback?select=id,user_id,subject,message,rating,featured,display_name,created_at&order=created_at.desc&limit=1000`);
      } catch (e) {
        testimonialColumnsMissing = true;
        all = await sbQuery(`feedback?select=id,user_id,subject,message,rating,created_at&order=created_at.desc&limit=1000`).catch(() => []);
        all = all.map(f => ({
          ...f,
          featured: false,
          display_name: null
      ...
      return res.status(200).json({
        feedbacks: enriched,
        total,
        ...(testimonialColumnsMissing ? {
          warning: 'featured/display_name columns not found on feedback table — run testimonials_supabase_schema.sql'
        } : {})
      });
    }
    if (action === 'feedback_feature') {
      ...
      try {
        ...
        return res.status(200).json({
          ok: true,
          feedback: updated?.[0] || null
        });
      } catch (e) {
        const hint = /column/i.test(e.message) ? ' — run testimonials_supabase_schema.sql to add the featured/display_name columns first' : '';
        return res.status(500).json({
          error: 'Failed to update feedback: ' + e.message + hint
        });
      }
    }
    if (action === 'feedback_stats') {
      const feedbacks = await cached('feedback_stats_raw', 60000, () => sbQuery('feedback?select=id,user_id,subject,message,rating,created_at&order=created_at.desc&limit=500').catch(() => []));
      const categories = {};
      const keywords = {
      ...
      const avgRating = ratedItems.length ? (ratedItems.reduce((s, f) => s + Number(f.rating), 0) / ratedItems.length).toFixed(1) : null;
      return res.status(200).json({
    ...
    if (action === 'retention') {
      ...
      const [allTests, allHours] = await cached('retention_raw', 60000, () => Promise.all([sbQuery(`tests?select=user_id,date&date=gte.${dateFrom(45)}`).catch(() => []), sbQuery(`hours?select=user_id,date&date=gte.${dateFrom(45)}`).catch(() => [])]));
      const userDates = {};
      [...allTests, ...allHours].forEach(r => {
        if (!r.date) return;
        if (!userDates[r.user_id]) userDates[r.user_id] = new Set();
      ...
      roster.forEach(u => {
        ...
        if (daysSinceSignup >= 1) {
          d1Eligible++;
          if (wasActiveOnDay(1)) d1Users.push(u.id);
        }
        if (daysSinceSignup >= 7) {
          d7Eligible++;
          if (wasActiveOnDay(7)) d7Users.push(u.id);
        }
        if (daysSinceSignup >= 30) {
          d30Eligible++;
          if (wasActiveOnDay(30)) d30Users.push(u.id);
        }
      });
      return res.status(200).json({
