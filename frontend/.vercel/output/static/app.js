...
window.JT_CACHE = {};
window.JT_MEMO = function(name, deps, fn) {
  const key = name + '_' + JSON.stringify(deps);
  if (window.JT_CACHE[key] !== undefined) return window.JT_CACHE[key];
  const res = fn();
  window.JT_CACHE[key] = res;
  return res;
};
window.JT_CLEAR_CACHE = function() { window.JT_CACHE = {}; };
...
async function handleGoogleCredential(response) {
  ...
  try {
    ...
    if (document.getElementById('auth-modal')) document.getElementById('auth-modal').classList.remove('active');
  } catch (err) {
    console.error("Google Auth Error:", err);
    showAuthErrPro(authTab, err.message);
  }
}
...
function save(){
  if(window.JT_CLEAR_CACHE) window.JT_CLEAR_CACHE();
  if(S.backlogStreak > 365) S.backlogStreak = 0;
  if(S.backlogBestStreak > 365) S.backlogBestStreak = 0;
...
function landingOpenAuth(mode) {
  ...
  if (wasOpen) {
    if (mode === 'signin') { authTab.click(); } else { authTabUp.click(); }
  } else {
    setTimeout(() => { if (mode === 'signin') { authTab.click(); } else { authTabUp.click(); } }, 10);
  }
  const viewport = document.querySelector('meta[name=viewport]');
  if (viewport) viewport.setAttribute('content', 'width=device-width, initial-scale=1, maximum-scale=1');
  
  // Render Google buttons after modal is fully displayed in DOM
  setTimeout(() => {
...
function updateCoachingGrid() {
  ...
  grid.innerHTML = list.map(c =>
    `<div class="ob-opt${(obData.mode==='hybrid' ? (obData.coaching||'').split(',').includes(c.id) : obData.coaching===c.id)?' sel':''}" onclick="obSelectCoaching('${c.id}')" data-coaching="${c.id}" style="padding:.55rem .5rem">
...
function buildSettingsCoachingSelect() {
  ...
  const userCoaching = userProfile.coaching || 'self';
  let optsHTML = [...list, { id:'self', name:'Self Study', sub:'' }]
    .filter((c,i,a)=>a.findIndex(x=>x.id===c.id)===i)
    .map(c => `<option value="${c.id}"${userCoaching===c.id?' selected':''}>${c.name}</option>`)
    .join('');

  if (userCoaching.includes(',')) {
    const names = userCoaching.split(',').map(id => COACHING_LIST.find(c=>c.id===id)?.name || id).join(' + ');
    optsHTML = `<option value="${userCoaching}" selected>${names}</option>` + optsHTML;
  }
...
function obSelectCoaching(id) {
  let selected = obData.coaching ? obData.coaching.split(',') : [];
  if (selected.includes(id)) {
    selected = selected.filter(x => x !== id);
    const el = document.querySelector(`[data-coaching="${id}"]`);
    if (el) el.classList.remove('sel');
  } else {
    if (selected.length >= 2) {
      let first = selected.shift();
      const el1 = document.querySelector(`[data-coaching="${first}"]`);
      if (el1) el1.classList.remove('sel');
    }
    selected.push(id);
    const el2 = document.querySelector(`[data-coaching="${id}"]`);
    if (el2) el2.classList.add('sel');
  }
  obData.coaching = selected.join(',');
  if (obData.coaching) _obClearHint(2);
}
...
async function sendFeedback() {
  ...
  try {
    ...
    if (!saved) {
      const mailtoUrl = `https://mail.google.com/mail/?view=cm&fs=1&to=5073340abdulrehmankhandurrani@gmail.com&subject=${encodeURIComponent('[JEE ADV OSINT Feedback] ' + subj)}&body=${encodeURIComponent(msg + '\n\n— Sent from JEE ADV OSINT\nUser: ' + (currentUser?.email || 'anonymous'))}`;
...
async function handleGoogleCredential(response) {
  ...
  try {
    ...
    if (error) throw error;
    if (document.getElementById('auth-modal-scrim')) closeAuthModal();
  } catch (err) {
    console.error("Google Auth Error:", err);
    showAuthErrPro(authTab, err.message);
  }
  ...
  containers.forEach(container => {
    if (container.childElementCount === 0) {
      google.accounts.id.renderButton(
        container,
        { theme: "filled_black", size: "large", type: "standard", shape: "rectangular", text: "continue_with", width: 280 }
      );
    }
  });
}
